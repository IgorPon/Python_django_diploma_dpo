...
class CompareTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create(
            email="test@ru.ru", password="admin732", first_name="test1", last_name="test1", phone="+7(999)999-99-99"
        )
        ...
        for num in range(1, 5):
            seller_product = SellerProduct.objects.create(
                seller=self.seller, product=Product.objects.get(id=num), price=100, quantity=10
            )
...
class HistoryOrderTest(TestCase):
    ...
    def setUp(self):
        ...
        self.order1 = Order.objects.create(
            customer=self.user, delivery="reg", payment_method="card", paid="True", in_order=True
        )
        self.order2 = Order.objects.create(
            customer=self.user, delivery="express", payment_method="cash", paid="False", in_order=True
        )
        ...
        for num in range(4):
            OrderProduct.objects.create(
                order=self.order2, seller_product=SellerProduct.objects.all()[num], final_price=100
            )

...
class Order(models.Model):
    ...
    customer = models.ForeignKey(
        User, on_delete=models.CASCADE, null=True, related_name="orders", verbose_name=_("customer")
    )
    ...
    phone_valid = RegexValidator(
        regex=r"^\+?1?\d{9,15}$",
        message=" ".join(
            [str(_("phone number must be entered in the format:")), "+999999999", str(_("up to 15 digits allowed."))]
        ),
    )
    phone = models.CharField(
        max_length=16, validators=[phone_valid], null=True, blank=True, verbose_name=_("phone number")
    )

    email = models.EmailField(null=True, blank=True, verbose_name=_("email"))

    delivery = models.CharField(max_length=3, choices=DELIVERY_CHOICES, default="reg", verbose_name=_("delivery"))

    payment_method = models.CharField(
        max_length=4, choices=PAYMENT_CHOICES, default="card", verbose_name=_("payment method")
    )
...
class OrderProduct(models.Model):
    ...
    final_price = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, verbose_name=_("Guess discount price")
    )

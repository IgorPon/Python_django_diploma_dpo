...
def change_limited_deal(request: HttpRequest) -> Callable:
    ...
    messages.add_message(
        request, settings.SUCCESS_OPTIONS_ACTIVATE, _("The Limited product has changed successfully.")
    )
    return redirect(request.META.get("HTTP_REFERER"))
...
def update_expire(request: HttpRequest) -> Callable:
    ...
    messages.add_message(
        request, settings.SUCCESS_OPTIONS_ACTIVATE, _("The Limited product days duration has changed.")
    )
    return redirect(request.META.get("HTTP_REFERER"))
...
def set_expire(request: HttpRequest) -> Callable:
    ...
    if dt > datetime.datetime.now():
        ...
        messages.add_message(
            request, settings.SUCCESS_OPTIONS_ACTIVATE, _("The time expire for limited deal was changed.")
        )
    else:
...
def clear_catalog_cache(request: HttpRequest) -> Callable:
    ...
    """
    cache.delete_many(
        ["products:all", "categories:all", "limited:all", "hot_offers:all", "stores:all", "random_categories:all"]
    )
    messages.add_message(request, settings.SUCCESS_OPTIONS_ACTIVATE, _("Cache was cleaned."))
    return redirect(request.META.get("HTTP_REFERER"))

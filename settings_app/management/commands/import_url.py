...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        for func, regex, url_name in view_functions:
            ...
            views.append(
                "{url}|{url_name}|{page}|{doc}|{methods}".format(
                    url_name=url_name, url=url, page=page, doc=doc, methods=methods
                ).strip()
            )
    ...
    def import_to_excel(self, data):
        ...
        for line in data:
            if (
                not line[0].startswith("/admin/")
                and not line[0].startswith("/accounts/")
                and not line[0].startswith("/i18nsetlang/")
                and not line[0].startswith("/__debug__/")
                and not line[0].startswith("/uploads/")
                and not line[0].startswith("/api_auth/")
                and not line[0].startswith("/swagger/")
            ):

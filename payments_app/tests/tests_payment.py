...
class PaymentTest(TestCase):
    ...
    def setUp(self):
        ...
        self.seller_1 = Seller.objects.create(name="test_seller_1", slug="test_seller_1", owner=self.user)

        self.seller_2 = Seller.objects.create(name="test_seller_2", slug="test_seller_2", owner=self.user)

        self.product_discount_1 = ProductDiscount.objects.create(
            name="test_product_discount_1",
        ...
        self.product_discount_2 = ProductDiscount.objects.create(
            name="test_product_discount_2",
        ...
        self.category_discount = GroupDiscount.objects.create(
            name="test_category_discount",
        ...
        for num in range(1, 3):
            Specifications.objects.create(value="num", current_specification=SpecificationsNames.objects.get(id=num))

        for num in range(1, 6):
            if num < 3:
                product = Product.objects.create(
                    name=f"name{num}", slug=f"name{num}", category=self.category_1, rating=1
                )
            else:
                product = Product.objects.create(
                    name=f"name{num}", slug=f"name{num}", category=self.category_2, rating=1
                )
        ...
        for num in range(1, 6):
            SellerProduct.objects.create(
                seller=self.seller_1, product=Product.objects.get(id=num), price=num * 100, quantity=100
            )

        for num in range(1, 6):
            SellerProduct.objects.create(
                seller=self.seller_2, product=Product.objects.get(id=num), price=num * 150, quantity=100
            )

        SellerProduct.objects.get(id=1).product_discounts.set([self.product_discount_1, self.product_discount_2])
    ...
    def test_pay_with_correct_account(self):
        ...
        for index in range(1, 6):
            OrderProduct.objects.create(
                order=order, seller_product=SellerProduct.objects.get(id=index), final_price=index * 100, quantity=1
            )
        order.in_order = True
        order.save()
        ...
        response = self.client.post(
            reverse("orders-polls:payment_with_account", args=[order.id]), {"numero1": 45621236}
        )
        self.assertEquals(response.status_code, 200)
    ...
    def test_pay_with_invalid_account(self):
        ...
        response = self.client.post(
            reverse("orders-polls:payment_with_account", args=[order.id]), {"numero1": 45621230}
        )
        self.assertEquals(response.status_code, 200)

    ...
    def setUpTestData(cls):
        ...
        for index in range(10):
            product = Product.objects.create(
                category=category, name=f"Product №{index}", slug=f"Product №{index}", is_published=True
            )
        ...
        for index in range(6):
            ...
            Banner.objects.create(
                discount=discount, title=f"Banner - {index}", description=f"Banner description {index}"
            )

...
class DiscountsTest(TestCase):
        ...
        for num in range(1, 6):
            if num < 3:
                product = Product.objects.create(
                    name=f"name{num}", slug=f"name{num}", image=temp_image, category=self.category_1, rating=1
                )
            else:
                product = Product.objects.create(
                    name=f"name{num}", slug=f"name{num}", image=temp_image, category=self.category_2, rating=1
                )
    ...
    def test_discounts_detail_page(self):
        ...
        response = self.client.get(
            reverse("discounts:discount-detail", args=[self.product_discount_1.slug, self.product_discount_1.id]),
            HTTP_REFERER="/discounts/",
        )

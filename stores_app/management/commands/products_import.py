...
class Command(BaseCommand):
    ...
    def create_discount(cls, discount: dict[str, any], seller: Seller, product: SellerProduct) -> None:
        ...
        discount = ProductDiscount.objects.get_or_create(
            seller=seller,
            set_discount=True,
    ...
    def save_log_info(cls, user: str, start_time: datetime, file_id: int) -> None:
        ...
        with open("import_log.txt", "r") as imp_log:
            result = imp_log.read()
            file_log = result[
                result.find(f"{user} starts import at {start_time}>") : result.find(
        ...
        for item_name, features in data.items():
            try:
                ...
                try:
                    ...
                    continue
                try:
                    new_product = self.update_product(
                        SellerProduct.objects.get(seller=seller, product=product),
                        features["price"],
                        features["quantity"],
                    )
                    logger.info("your product was updated>")
                except ObjectDoesNotExist:
                    new_product = SellerProduct.objects.create(
                        seller=seller,
                        product=product,

...
class StoresTestCase(TestCase):
    ...
    def setUpTestData(cls):
        cls.user = User.objects.create_user(
            email="test@user.com", password="testp@sw0rd", first_name="user", last_name="test", phone="9222222222"
        )
        ...
        Seller.objects.bulk_create(
            [
                Seller(name="test_store", slug="test_store_1", owner=cls.user),
        ...
        Product.objects.bulk_create(
            [
                Product(name=f"name{num}", slug=f"name{num}", category=random.choice(categories), rating=1)
        ...
        SellerProduct.objects.bulk_create(
            [
                SellerProduct(seller=seller, product=Product.objects.get(id=num), price=num * 100, quantity=100)
                for num in range(1, 4)
            ]
        )
    ...
    def test_no_get_seller_room(self):
        ...
        testuser = User.objects.create(
            email="test2@user.com", password="testp@sw0rd2", first_name="user2", last_name="test2", phone="9333333333"
        )
